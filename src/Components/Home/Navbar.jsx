...
const NavBar = () => {
 ...
 <div className= 'fixed top-0 z-40 bg-[#ffffff06] backdrop-blur-md  lg:static w-full bg-white flex lg:flex-row flex-col justify-between px-3 py-4 lg:px-28 lg:py-4 shadow-sm '>
   ...
   <div ref={sidebarRef} className={`flex lg:h-auto h-[100vh] lg:bg-transparent bg-white lg:flex-row flex-col lg:justify-between items-center  lg:gap-28 lg:static  absolute lg:w-3/5 w-[70%] border-r-[1px] top-0  transition-all duration-700 delay-100 ease-in-out  ${showNav ? "left-0" : "-left-96"}`}>
   ...
   <div className='flex lg:flex-row flex-col gap-3 mb-10 mt-28 lg:my-0 justify-start w-32 lg:w-auto'>
   ...
   <Link to={user? "dashboard" : "signup"} onClick={handleShowNav}  >
    <span className='flex justify-center items-center text-cyan-600  w-2/3 lg:w-autopx-2 py-1 shadow-md shadow-gray-300 border border-cyan-600 rounded-lg text-sm hover:border-cyan-300 hover:shadow-cyan-300 hover:bg-cyan-300 hover:text-white hover:scale-105  transition-all delay-75 duration-300 ease-in-out  ' >
      SIGN UP
    </span>
   </Link>
   </div>
