...
export default function MobileView({ sitedata }){
  const { user, links, getLinks, description } = useAuth();
  const prflName = user.name;

  return (
    <div className="hidden xl:block h-screen w-[25%]">
              ...
              {links.map((data: any) => (
                ...
                >
                  <div className="flex gap-3 items-center">
                    <span
                      className="w-8 h-8 rounded bg-no-repeat bg-cover"
                      style={{
