    ...
    <div className='flex flex-col gap-10 md:flex-row md:justify-between md:items-center py-5 px-2  '>
         ...
         <div className='flex gap-5 items-center justify-center  w-full md:w-[70%] lg:w-[60]'>
          ...
          <span className='text-[1.3rem] text-slate-400/60 md:static absolute top-[1.4rem] right-20 '><FontAwesomeIcon icon={faBell} /></span>
            <span onClick={handleShow} className={`${!user.profileImg ? "bg-cyan-600" : null } z-20 justify-center font-bold text-2xl md:w-16 w-14 h-14 md:h-16 rounded-full absolute top-2 right-3 md:static flex items-center bg-no-repeat bg-cover border-2 border-white capitalize`} style={{backgroundImage: `url(${user.profileImg})`}}>{!user.profileImg ? user.name.charAt(0) : null}</span>
