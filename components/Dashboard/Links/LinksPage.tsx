...
const LinkPage:FC<Props> = ({ siteInfo }) => {
  ...
  const [siteLink, setSiteLink] = useState<string>(siteInfo.siteLink);
  const [showAddImg, setShowAddImg] = useState<boolean>(false);
  const [loading, setLoading] = useState<boolean>(false);
  const [profileImg, setProfileImg] = useState<File | null>(null);
  const [previewImg, setPreviewImg] = useState<string>("");
  const [showSavePhoto, setShowSavePhoto] = useState<boolean>(false);
  const imgInputRef = useRef<HTMLInputElement>(null);
  const linkImg:string = siteInfo.linkImg;
        ...
        <div className="flex w-full h-full top-0 z-20 bg-cyan-600/10 flex-wrap items-center gap-2 justify-center absolute px-3 lg:px-0">
          <div className="lg:w-2/5 w-full bg-white rounded-md flex flex-col gap-3 items-center justify-center p-3">
            <span className="w-full flex items-center justify-end">
              <span
                className="p-2 text-xl w-7 h-7 flex items-center justify-center transition-all duration-300 delay-75 ease-in-out hover:bg-gray-200 rounded-full"
            ...
            <span
              className="flex bg-cover bg-no-repeat items-center justify-center w-72 h-72"
              style={{ backgroundImage: `url(${previewImg})` }}
            ></span>
            <button
              className="px-2 py-1 rounded-md w-28 border bg-cyan-600 text-white transition-all duration-200 delay-75 ease-in-out hover:bg-cyan-700 hover:scale-110"
      ...
      {showDelete ? (
        ...
        >
          <div
            ...
            className="w-4/5 md:w-2/5 bg-white flex flex-col gap-6 md:gap-4 justify-center items-center p-5 rounded-lg shadow-lg border"
          >
            ...
            <span className="flex gap-5 w-full items-center justify-center">
              <button
                className="hover:bg-gray-600/60 w-20 bg-gray-300 text-white p-1 rounded-md transition-all duration-200 delay-200"
                onClick={handleShowDeleteModal}
              >
                Cancel
              </button>
              <button
                className="hover:bg-red-600/60 w-20 bg-red-600 text-white p-1 rounded-md transition-all duration-200 delay-200"
